...
Usage:
    python experiments/robot/libero/regenerate_libero_dataset.py \
        --libero_task [ libero_spatial | libero_object | libero_goal | libero_10 ] \
        --libero_datapath <PATH TO RAW HDF5 DATASET DIR> \
        --regenerate_path <PATH TO TARGET DIR>

    Example (LIBERO-Spatial):
        python experiments/robot/libero/regenerate_libero_dataset.py \
            --libero_task libero_spatial \
            --libero_datapath ./LIBERO/libero/datasets/libero_spatial \
            --regenerate_path ./LIBERO/libero/datasets/libero_spatial_no_noops
...
from datasets.libero.libero_utils import (
...
def main(args):
    print(f"Regenerating {args.libero_task} dataset!")
    args.libero_datapath = os.path.join(args.libero_datapath, args.libero_task)
    args.regenerate_path = os.path.join(args.regenerate_path, args.libero_task)
    # Create target directory
    if os.path.isdir(args.regenerate_path):
        user_input = input(f"Target directory already exists at path: {args.regenerate_path}\nEnter 'y' to overwrite the directory, or anything else to exit: ")
        if user_input != 'y':
            exit()
    os.makedirs(args.regenerate_path, exist_ok=True)

    # Prepare JSON file to record success/false and initial states per episode
    metainfo_json_dict = {}
    metainfo_json_out_path = f"datasets/libero/{args.libero_task}_metainfo.json"
    ...
    task_suite = benchmark_dict[args.libero_task]()
    ...
    for task_id in tqdm.tqdm(range(num_tasks_in_suite)):
        ...
        orig_data_path = os.path.join(args.libero_datapath, f"{task.name}_demo.hdf5")
        assert os.path.exists(orig_data_path), f"Cannot find raw data file {orig_data_path}."
        orig_data_file = h5py.File(orig_data_path, "r")
        orig_data = orig_data_file["data"]

        # Create new HDF5 file for regenerated demos
        new_data_path = os.path.join(args.regenerate_path, f"{task.name}_demo.hdf5")
    ...
    print(f"Dataset regeneration complete! Saved new dataset at: {args.regenerate_path}")
