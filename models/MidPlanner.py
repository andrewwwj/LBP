...
import random
import pickle
from .components.MetaTask import LatentDynamics
...
class DnceLatentProj(nn.Module):
    def __init__(self, latent_info_file='assets/libero.pkl'):
        ...
        if latent_info_file is not None:
            with open(latent_info_file, "rb") as f:
                data = pickle.load(f)
            img0_mean = torch.tensor(data['img_latents_dnce']['image0']['mean'])
            img0_std = torch.tensor(data['img_latents_dnce']['image0']['std'])
            img1_mean = torch.tensor(data['img_latents_dnce']['image1']['mean'])
            img1_std = torch.tensor(data['img_latents_dnce']['image1']['std'])
        else:
            img0_mean = torch.zeros(1024)
            img0_std = torch.ones(1024)
            img1_mean = torch.zeros(1024)
            img1_std = torch.ones(1024)

        self.register_buffer('img_mean', img0_mean)
        self.register_buffer('img_std', img0_std)
        self.register_buffer('img1_mean', img1_mean)
        self.register_buffer('img1_std', img1_std)
    ...
    def img_proj(self, x, view='main'):
        # x = self.add_learnable_noise(x)
        if view=='main':
            img_mean = self.img_mean
            img_std = self.img_std
        elif view=='wrist':
            img_mean = self.img1_mean
            img_std = self.img1_std
        else:
...
class MidImaginator(nn.Module):
    ...
    def forward(self, cur_images, instruction, sub_goals, **kwargs):
        ...
        sub_goals = self.latent_proj.img_proj(sub_goals, view='main')
        ...
        img_latent_hist = []
        for t in range(T):
            img = images_history[:, t, 0, ...]  # third view only
        ...
        # -------------- 2) Multi-view --------------
        # view_means = torch.stack([self.latent_proj.img_mean, self.latent_proj.img1_mean], dim=0)  # [V, D]
        # view_stds = torch.stack([self.latent_proj.img_std, self.latent_proj.img1_std], dim=0)  # [V, D]
        # imgs_flat = rearrange(images_history, 'b t v c h w -> (b t v) c h w')  # [(B*T*V), C, H, W]
        # z = self.latent_proj.latent_proj.model.encode_image(imgs_flat)  # [(B*T*V), D]
        # BT = images_history.shape[0] * T
        # idx = torch.arange(V, device=z.device).unsqueeze(0).expand(BT, V).reshape(-1)  # [(B*T*V)]
        # means = view_means.to(z.device, z.dtype)[idx]
        # stds = view_stds.to(z.device, z.dtype)[idx].clamp_min(1e-6)  # safe divide
        # z = (z - means) / stds
        # img_latent_history = rearrange(z, '(b t v) d -> b t (v d)', v=V, t=T)
        # --------------------------------------------
        nearest_subgoal = sub_goals[:, -1, ...]
        # IK forward
        _, ik_loss_dict = self.latent_dynamics(
    ...
    def generate(self, image_history, instruction, recursive_step, proprios_history, prev_action):
        ...
        s0_history = torch.stack([self.latent_proj.img_proj(image_history[:, t, ...]) for t in range(T)], dim=1)   # [B, T, 1024]
        s0 = s0_history[:, -1]
        sg = self.latent_proj.lang_proj(instruction)
